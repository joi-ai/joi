...
def initDetector(joi):
    ...
    if config.get("detector", "snowboy") == "porcupine":
        ...
        try:
            while True:
                ...
                if result >= 0:
                    ...
                    joi._detected_callback(False)
                    recorder.stop()
                    joi.conversation.interrupt()
                    query = joi.conversation.activeListen()
        ...
        models = constants.getHotwordModel(config.get("hotword", "joi.pmdl"))
        detector = snowboydecoder.HotwordDetector(
            models, sensitivity=config.get("sensitivity", 0.5)
        )
        # main loop
        try:
            callbacks = joi._detected_callback
            detector.start(
                detected_callback=callbacks,
                audio_recorder_callback=joi.conversation.converse,
                interrupt_check=joi._interrupt_callback,
