...
class MainHandler(BaseHandler):
    def get(self):
        global conversation, joi, suggestions
...
def run(conversation, joi, debug=False):
    settings["debug"] = debug
    t = threading.Thread(target=lambda: start_server(conversation, joi))
    t.start()
